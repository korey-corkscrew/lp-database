...
import { PoolLookupDatabase } from "./pool-lookup-database";
...
export class PoolDatabase {
    ...
    public static async getAllPoolAddresses(
        _chainId: number,
        _factory?: string
    ) {
        const pools = await this.poolData.find(
            {
                chainId: _chainId,
                factory: _factory,
            },
    ...
    public static async getLastCreatedPoolBlock(
        _chainId: number,
        _factory?: string
    ) {
        const pools = await this.poolData
            .find({ chainId: _chainId, factory: _factory })
            .sort({ blockCreated: -1 });
    ...
    public static async getLastUpdatedPoolBlock(
        _chainId: number,
        _factory?: string
    ) {
        const pools = await this.poolData
    ...
    public static async getFirstUpdatedPoolBlock(
        _chainId: number,
        _factory?: string
    ) {
        const pools = await this.poolData
    ...
    public static async updatePoolReserves(
        ...
        if (_factory) {
            await this.poolData.findOneAndUpdate(
                { _id: id, factory: _factory },
                {
